...
const workflow = readFileSync(join(root, ".github", "workflows", "native-x64.yml"), "utf8");

describe("release configuration", () => {
  test("pins Electron and uses electron-builder for every x64 package", () => {
  ...
  test("gates tagged publishing on tested native packages", () => {
    ...
    expect(workflow).toContain("needs: [prepare, build-smoke, compatibility]");
    expect(workflow).toContain("SHA256SUMS");
    expect(workflow).toContain("actions/attest-build-provenance@v3");
    expect(workflow).toContain("gh release create");
    expect(workflow).toContain("gh release edit");
    expect(workflow).toContain("Smoke every native package");
    expect(workflow).toContain("--target appimage --host-version 0.1.1-rc.2");
    expect(workflow).toContain("Latest Host compatibility");
  });
});
